...
/*   Updated: 2021/08/24 15:34:00 by bclerc           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	check_if_ra_b(t_push *push, int part, t_median median)
{
	...
	while (tmp)
	{
		if (part == 0 && tmp->value <= median.quarter)
			return (1);
		if (part == 1 && tmp->value > median.quarter && tmp->value <= median.median)
			return (1);
		if (part == 2 && tmp->value > median.median && tmp->value <= median.three_quarter)
			return (1);
...
void	push_median_bb(t_push *push, int *size, int party, t_median median)
{
	while (size > 0 && check_if_ra_b(push, party, median))
	{
		if (party == 0)
		{
			if ((*push->stacka)->value <= median.quarter)
			{
		...
		else if (party == 1  && (*push->stacka)->value > median.quarter && (*push->stacka)->value <= median.median)
		{
			pushs(push->stacka, push->stackb);
			ft_putstr("pb\n");
			continue ;
		}
		else if (party == 2  && (*push->stacka)->value > median.median && (*push->stacka)->value <= median.three_quarter)
		{
			pushs(push->stacka, push->stackb);
			ft_putstr("pb\n");
			continue ;
